...
export interface typeInputTextProps
  extends Omit<
  ...
  label?: ReactNode;
  setter: Dispatch<
    SetStateAction<typeStateInput | ((prev: typeStateInput) => void)>
  >;
...
function InputText({
  ...
  function onChangeValue(e: FormEvent<HTMLInputElement>) {
    ...
    setter((prev: typeStateInput) => {
      ...
      const invalidMessages: string[] = validations
        ? validator({ validations, value: valueInput })
        : [];
  ...
  return (
    <div
      className={`input-group ${
        getter?.invalids?.[name]?.length ? "input-group-invalid" : ""
      } ${className}`}
    >

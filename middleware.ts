...
export function middleware(request: NextRequest) {
    ...
    if (['login'].includes(prefix)) { // --> route guest
        if (userToken) {
            return NextResponse.redirect(new URL('/', request.url))
        }
    } else { // --> route authed
        if (!userToken) {
            ...
            try {
                ...
                featureAccesses = JSON.parse(cookieUserAuth)?.access?.ER?.featureAccesses
            } catch (error) { }
            ...
            switch (prefix) {
                case 'virtual-rapor':
                    if (!checkAccessFeature(['show-self-virtual-rapor@ER>=1'], featureAccesses))
                        return abort();
                    break;

                case 'kelas':
                    if (!checkAccessFeature([
                        'show-all-data-class@ER>=1',
                        'show-data-class-taughted@ER>=1'
                    ], featureAccesses))
                        return abort();
                    break;
